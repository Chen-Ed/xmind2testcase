...
from webtool.xmind2case.utils import get_absolute_path, xmind_testcase_to_json_file
...
using_doc = """
    ...
    Usage:
     xmind2case [path_to_xmind_file] [-csv] [-xml] [-json]
     xmind2case [webtool] [port_num]
    
    Example:
     xmind2case /path/to/testcase.xmind        => output testcase.csv、testcase.xml、testcase.json
     xmind2case /path/to/testcase.xmind -csv   => output testcase.csv
     xmind2case /path/to/testcase.xmind -xml   => output testcase.xml
     xmind2case /path/to/testcase.xmind -json  => output testcase.json
     xmind2case webtool                        => launch the web testcase conversion tool locally: 127.0.0.1:5001
     xmind2case webtool 8000                   => launch the web testcase conversion tool locally: 127.0.0.1:8000
    """
